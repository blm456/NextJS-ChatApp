...
const FriendRequestSidebarOptions: FC<FriendRequestSidebarOptionsProps> = ({
    ...
    useEffect(() => {
        ...
        const friendRequestHandler = async () => {
            const result = await axios.get('/api/friends/add')
            setUnseenRequestCount(result.data.count)
        }

        const addedFriendHandler = async () => {
            const result = await axios.get('/api/friends/add')
            setUnseenRequestCount(result.data.count)
        }

        const denyFriendHandler = async () => {
            const result = await axios.get('/api/friends/add')
            setUnseenRequestCount(result.data.count)
        }
    ...
    return (
        <Link
            ...
            className='text-gray-700 hover:text-indigo-600 hover:bg-gray-50 group flex items-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold transition'
        >
            <div className='text-gray-400 border-gray-200 group-hover:border-indigo-600 group-hover:text-indigo-600 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border text-[0.625rem] font-medium bg-white'>
                <User className='w-4 h-4' />
            </div>
            <p className='truncate'>Friend Requsts</p>
